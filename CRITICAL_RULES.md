...
2. ARTICLE BODY TEXT: Do not reword, paraphrase, summarize, or clean up. Preserve source text exactly. The pipeline **never** sends the article HTML body through Claude: it extracts the body directly from the Google Doc HTML export in code, applies deterministic link normalization for Cultural Daily, and **ignores** any ``article_body_html`` field in the model response. Claude only receives a short plaintext excerpt for metadata (SEO title, meta description, focus keyword, topic slug, hero query), not the raw Doc HTML body.
